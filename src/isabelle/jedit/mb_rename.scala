      ...
      Document_View.get(text_area).foreach { doc_view =>
        ...
        newName match {
          case Some(name) if name.nonEmpty =>
            // Do rename
            rename_in_buffer(text_area.getBuffer, rangeList, name)
          case _ =>
            // Cancelled or left blank
        }
      }

      // TODO: the defining occurrence in definition body is not captured by entity
      // markup and so is not renamed. A separate cumulate pass looking for the definition-site
      // markup on the same canonical_id is probably the right solution.
      // Requires inspecting what markup Isabelle attaches to the definition header.
    }
  def rename_in_buffer(buffer: JEditBuffer, ranges: List[Text.Range], newName: String): Unit =
    ranges.sortBy(_.start).reverse.foreach { r=>
      buffer.remove(r.start, r.stop - r.start)
      buffer.insert(r.start, newName)
    }
